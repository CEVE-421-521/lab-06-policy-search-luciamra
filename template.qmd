...
```{julia}
function opt9mization_objective()
    #∆h = 1.0:0.5:14.0
    opt_obj_res = []
    #∆h_res = []
    #for h in ∆h
        ...
        # Optomization:
        result = optimize(objective(a), bounds)
        # Minimum:
        minimum(result)
        # Value of the decision variable that achieves the minimum:
        minimizer(result)
        ...
        # Random Seed:
        Random.seed!(2024)
        result2 = optimize(objective(a), bounds, algorithm)
        push!(opt_obj_res, result2)

    #end
    return opt_obj_res
end

x =optimization_objective()
```

This optimization function is not working for me, with the following error:
no method matching optimize(::Float64, ::BoxConstrainedSpace{Int64})
This does not allow me to continues with the project at this moment. We can however gage some information from the trials for the objective(a) function, which considered elevations of 1ft, 7ft, and 13ft. Between these three actions, the one with the lowest npv value is 7ft. demonstrating that the best options are in the middle.
