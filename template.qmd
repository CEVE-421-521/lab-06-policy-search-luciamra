...
```{julia}
function optomization_objective()
    ∆h = 1.0:0.5:14.0
    opt_obj_res = []
    ∆h_res = []
    for h in ∆h
        a = Action(h*u"ft")
        #bounds = boxconstraints(; lb=-999_999_999, ub=999_999_999)
        # Optomization:
        result = optimize(objective(a))
        # Minimum:
        minimum(result)
        # Value of the decision variable that achieves the minimum:
        minimizer(result)
        ...
        # Random Seed:
        Random.seed!(2024)
        result2 = optimize(objective(a), algorithm)
        push!(opt_obj_res, result2)
    end
    return opt_obj_res
end

x =optomization_objective()
```